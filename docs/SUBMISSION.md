...
| **KPI design relevance and originality** | True Efficiency formulas, validated to separate healthy/failing by 114.4% vs ~14% for naive J/TH | `src/kpi/true_efficiency.py`, `TECHNICAL_REPORT.md` §4, `01_eda.executed.ipynb` §5 |
| **Feasibility of AI use case** | Per-failure-type detection table shows 5 of 6 test failures caught with 7.6-day avg lead time; head-to-head vs threshold baseline shows 6× better signal-to-noise | `TECHNICAL_REPORT.md` §5.3, §5.5; `02_results.executed.ipynb` |
